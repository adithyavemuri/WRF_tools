...
import numpy as np
import xarray as xr
from .grid import interpolate_to_levels, nearest_grid_point

def point(dataset: xr.Dataset, latitude: float, longitude: float, *, variables=None, time=None, lat_name="XLAT", lon_name="XLONG") -> xr.Dataset:
...
def vertical_profile(dataset: xr.Dataset, variable: str, *, x: int, y: int, time=0) -> xr.DataArray:
...
def horizontal_plane(dataset: xr.Dataset, variable: str, *, level=None, time=0) -> xr.DataArray:
    data=dataset[variable]; indexers={}
